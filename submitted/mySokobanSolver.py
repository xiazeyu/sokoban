...
def _taboo_cells_solver(warehouse):
    """
...
class State:
    ...
    def __init__(self, worker = None, boxes = None):
        self.worker = worker
...
class SokobanPuzzle(search.Problem):
    ...
    def parse_goal_node(self, goal_node):
        """
    ...
    def print_worker_cost(self, state):
        """
    ...
    def _dist_map(self, state, start):
        """
        Return a 2D array of the cost of moving the worker to each position.
        ...
        """
        # initialize
        dist = [[math.inf for _ in range(self.nrows)]
                                 for _ in range(self.ncols)]
        final = [[False for _ in range(self.nrows)]
                                   for _ in range(self.ncols)]
    ...
    def uniform_cost_search_h(self, node):
        # no heuristic
        # this becomes uniform_cost_search
        return 0

    def nearest_manhattan_sum_h(self, node):
    ...
    def permutation_manhattan_sum_h(self, node):
        ...
        for match in matches:
            ...
            for index, (box, target) in enumerate(match):
                match_cost += manhattan_distance(box,
                                                 target) * (self.weights[index] + 1)
            cost = min(cost, match_cost)

        return cost / len(node.state.boxes)

    # def hungarian_manhattan_sum_h(self, node):
    ...
    #             cost_matrix[g_idx, b_idx] = manhattan_distance(
    #                 self.targets[g_idx], boxes[b_idx]) * (self.weights[b_idx]+1)
    ...
    # def hungarian_dijkstra_sum_h(self, node):
    ...
    #     for g_idx in range(len(self.targets)):
    #         for b_idx in range(len(boxes)):
    #             goal = self.targets[g_idx]
    #             box = boxes[b_idx]
    #             cost_matrix[g_idx, b_idx] = self._dist_map(
    #                 node.state, box)[goal[0]][goal[1]] * (self.weights[b_idx]+1)
    #     # print(cost_matrix)
    #     row_ind, col_ind = scipy.optimize.linear_sum_assignment(cost_matrix)
    #     cost = cost_matrix[row_ind, col_ind].sum()
